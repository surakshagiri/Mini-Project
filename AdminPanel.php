	...
	<?php
		...
		if (!$con) {
			...
			$airline=$_POST['airline'];
				...
				$dltsql="Delete from flightdetails where(Date,FlightNumber,Time) IN ( Select Date,FlightNumber,Time from flightdetails where Airline ='$airline' and Date='$datepicker' and FlightNumber='$airlineno' and Time ='$time' and Source='$source' and Destination='$destination') ";
				$res=mysqli_query($con,$dltsql);
				$aff = mysqli_affected_rows($con);
				if ($aff>0) {
					$dltsql1="Delete from passengerdetails where(Date,FlightNumber,Time) In (Select Date,FlightNumber,Time from passengerdetails where Airline ='$airline' and Date='$datepicker' and FlightNumber='$airlineno' and Time ='$time' and Source='$source' and Destination='$destination') ";
					$res1=mysqli_query($con,$dltsql1);
					$aff1= mysqli_affected_rows($con);
					if ($aff1>0) {
						$msg="All details Deleted Successfully";
					}
					else
					{
					...
					$msg="No Flights Available To Delete";
				}
		}
